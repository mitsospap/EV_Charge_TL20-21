...
const fs = require('fs');
const { checkId, check_apikey, get_username, get_token, get_sessions_ev } = require('../../functions');
// require("dotenv").config();

program
    .version('0.0.1')
    .requiredOption('--ev, <number>', 'choose an event id')
                ...
                get_username((err, username) => {
                    if (err) {
                        ...
                        check_apikey(username, options.apikey).then(result => {
                            if (result === "OK") {
                                get_sessions_ev(options.ev, options.datefrom, options.dateto, token, format, options.format).then(response => {
