...
const fs = require('fs');
const { checkId, check_apikey, get_username, get_token, get_sessions_point } = require('../../functions');
// require("dotenv").config();

program
    .version('0.0.1')
    .requiredOption('--point, <number>', 'choose an event a point id')
                ...
                get_username((err, username) => {
                    if (err) {
                        ...
                        check_apikey(username, options.apikey).then(result => {
                            if (result ="OK") {
                                get_sessions_point(options.point, options.datefrom, options.dateto, token, format, options.format).then(response => {
